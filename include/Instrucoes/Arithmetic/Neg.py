def NEG(code_base, code_limit, data_base, data_limit, cs_selector, ds_selector, ss_selector):
    """
    Simulação de uma instrução NEG em modo Protegido (32 bits).
    ...
    """
    print("\nPara a instrução NEG, informe a instrução completa no formato:")
    print("   <OFFSET_INSTRUÇÃO> NEG <DST_OFFSET>")
    print("Exemplo: 00024000 NEG 00040000")
    instr_input = input("   - Instrução: ").strip()

    # Tenta separar os componentes da linha de comando.
    try:
        # Exemplo de entrada: "00024000 NEG 00040000"
        ...
        # O segundo deve ser o mnemônico (NEG)
    ...
    registers = {
        ...
        "DST": hex(dst_offset),
        "EFLAGS": {"ZF": 0, "SF": 0, "OF": 0}  # Flags relevantes para a operação NEG
    }

    print("\n==============================================")
    print("Simulação da Execução da Instrução NEG")
    print("==============================================\n")
    ...
    # -- Etapa 4: Execução da Instrução NEG --
    print("\nEtapa 4: Execução da Instrução NEG")
    resultado = 0 - dst_data_value  # Nega o valor (DST = 0 - DST)
    
    # Atualiza as flags com base no resultado da negação
    ...
    registers["EFLAGS"]["OF"] = 1 if dst_data_value == -2**31 else 0  # OF = 1 se houver overflow (caso especial para -2^31)
    
    print(f"   -> Resultado da NEG: {hex(resultado)}")
